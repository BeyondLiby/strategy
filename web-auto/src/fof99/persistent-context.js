...
export async function launchFof99Context(config, rootDir, options = {}) {
  const userDataDir = fof99ProfileDir(config, rootDir);
...
export function fof99ProfileDir(config, rootDir) {
  return path.join(rootDir, config.session?.profileDir || "runtime/fof99-profile");
}
